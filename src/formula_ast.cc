...
FormulaAst::AstPtr FormulaAst::parse(
  ...
  if ( type == ParserType::TFormula ) {
    ...
    return std::make_shared<FormulaAst>(
        translate_tformula_ast(tformula_parser.parse(expression), TranslationContext{params, variableIdx, bind_parameters})
        );
  }
  throw std::runtime_error("Unrecognized formula parser type");
}

double FormulaAst::evaluate(const std::vector<Variable::Type>& values) const {
  switch (nodetype_) {
    ...
    case NodeType::Parameter:
      throw std::runtime_error("FormulaAst has a parameter node but was not evaluated with parameters supplied");
    case NodeType::UAtom:
      switch (std::get<UnaryOp>(data_)) {
        case UnaryOp::Negative: return -children_[0].evaluate(values);
      }
    case NodeType::UnaryCall:
      return std::get<UnaryFcn>(data_)(
          children_[0].evaluate(values)
          );
    case NodeType::BinaryCall:
      return std::get<BinaryFcn>(data_)(
          children_[0].evaluate(values), children_[1].evaluate(values)
          );
    case NodeType::Expression:
      auto left = children_[0].evaluate(values);
      auto right = children_[1].evaluate(values);
